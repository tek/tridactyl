...
import {inContentScript} from "@src/lib/webext"
...
const state = (new Proxy(overlay, {
    ...
    get(target, property) {
        if (inContentScript()) throw "State object must be accessed with getAsync in content"
...
export async function getAsync(property) {
    if (inContentScript()) return browser.runtime.sendMessage({type: "state", command: "stateGet", args: [{prop: property}]})
...
messaging.addListener("state", (message, sender, sendResponse) => {
    if (message.command == "stateUpdate") Object.assign(overlay, message.args[0].state)
    else if (message.command == "stateGet") return state[message.args[0].prop]
